...
def main():
    ...
    with st.expander("ℹ️ 스코어 산출 공식 및 로직 설명"):
        ...
        **측정 방법**:
        - 평균 댓글 수가 많을수록 높은 점수
        - 댓글이 거의 없는 게시물(3개 이하)이 많으면 페널티
        
        **예시**:
        - 평균 댓글 20개 이상 → 최고점 (50점)
        - 댓글 없는 게시물이 80% 이상 → -15점 페널티
        ...
        **측정 방법**:
        - 최근 활동일이 가까울수록 높은 점수
        - 90일 내 포스팅 수가 많을수록 높은 점수
        - 비공개 계정은 감점
        
        **예시**:
        - 7일 이내 활동 + 90일간 5개 이상 포스팅 → 높은 점수
        - 30일 이상 미활동 → 낮은 점수
        
        ---
        
        #### 🏃 RunnerFit (러닝 적합도, 20점)
        **의미**: 러닝 관련 콘텐츠를 주로 올리는가?
        
        **측정 방법**:
        - 게시물 캡션/해시태그에 러닝 키워드 포함 비율
        - 키워드: 러닝, 달리기, 마라톤, run, 조깅, 트레일, 울트라
        
        **예시**:
        - 5개 게시물 모두 러닝 관련 → 20점 (만점)
        - 5개 중 3개만 러닝 관련 → 12점
        ...
        | 컬럼명 | 계산 방법 | 해석 |
        |--------|-----------|------|
        | **참여율(%)** | (평균댓글 ÷ 팔로워) × 100 | 팔로워 대비 소통 효율성 (높을수록 좋음) |
        | **댓글/좋아요비율** | 평균댓글 ÷ 평균좋아요 | 높을수록 소통 활발 (0.05 이상 권장) |
        | **저댓글비율** | 댓글 3개 이하 게시물 / 전체 5개 | 낮을수록 좋음 (0이면 모든 글에 댓글 4개+) |
        | **러닝태그율** | 러닝 관련 게시물 / 전체 게시물 | 1.0이면 100% 러닝 계정 |
