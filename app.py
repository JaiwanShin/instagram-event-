...
def main():
    ...
    with st.expander("ℹ️ 스코어 산출 공식 및 로직 설명"):
        ...
        > **Final Score = Relationship (50) + Reliability (30) + RunnerFit (20)**
        
        ---
        
        ## 📊 1. Relationship (관계성, 50점)
        
        ### 의미 (Why)
        팔로워와 **얼마나 진정성 있게 소통**하는가를 측정합니다.
        - 댓글이 많다 = 활발한 소통
        - **팔로워 대비 참여율이 높다 = 효율적 소통** (🆕 Engagement Rate 반영)
        
        ### 수식 (How)
        ```
        Relationship = 댓글 기반 점수(30) + Engagement Rate 점수(20) + 페널티
        ```
        
        | 구성 요소 | 구간 | 점수 |
        |-----------|------|------|
        | **댓글 기반 (30점)** | 평균 댓글 ≥4개 | 30점 |
        | | 2~4개 | 21점 |
        | | 1~2개 | 12점 |
        | | 0.5~1개 | 6점 |
        | | <0.5개 | 3점 |
        | **Engagement Rate (20점)** | ≥1.0% | 20점 |
        | | 0.5~1.0% | 15점 |
        | | 0.2~0.5% | 10점 |
        | | 0.1~0.2% | 5점 |
        | | <0.1% | 0점 |
        ...
        ### 예시 (Example)
        - **팔로워 1,000명 + 평균 댓글 10개**: Engagement Rate = 1.0% → 20점
        - **팔로워 10,000명 + 평균 댓글 10개**: Engagement Rate = 0.1% → 5점
        
        > 💡 **같은 댓글 수라도 팔로워 대비 참여율이 높은 사람이 더 높은 점수!**
        
        ---
        
        ## 🔄 2. Reliability (신뢰성, 30점)
        
        ### 의미 (Why)
        **꾸준히 활동하는 계정**인지를 측정합니다.
        - 최근에 포스팅함 = 활성 계정
        - 90일간 포스팅 수가 많음 = 지속적 활동
        - 비공개 계정 = 미션 확인 어려움 (감점)
        
        ### 수식 (How)
        ```
        Reliability = (최근 활동 점수 + 90일간 포스트 점수) / 2 + 비공개 페널티
        ```
        ...
        ### 예시 (Example)
        - **3일 전 활동 (30점) + 90일간 5개 포스트 (30점) = 평균 30점**
        - **45일 전 활동 (10점) + 90일간 1개 포스트 (10점) = 평균 10점**
        
        ---
        
        ## 🏃 3. RunnerFit (러닝 적합도, 20점)
        
        ### 의미 (Why)
        **러닝 관련 콘텐츠를 주로 올리는 사람**인지를 측정합니다.
        - 러닝 키워드가 포함된 게시물 비율을 계산
        
        ### 수식 (How)
        ```
        RunnerFit = 러닝 관련 게시물 수 / 전체 게시물 수
        ```
        ...
        **러닝 키워드**: 러닝, 달리기, 마라톤, run, 조깅, running, 트레일, 울트라
        ...
        | 컬럼명 | 계산 방법 | 해석 |
        |--------|-----------|------|
        | **참여율(%)** | (평균댓글 ÷ 팔로워) × 100 | 높을수록 효율적 소통 |
        | **댓글/좋아요비율** | 평균댓글 ÷ 평균좋아요 | 0.05 이상 권장 |
        | **저댓글비율** | 댓글 3개 이하 게시물 / 전체 | 낮을수록 좋음 |
        | **러닝태그율** | 러닝 관련 게시물 / 전체 | 1.0 = 100% 러닝 계정 |
